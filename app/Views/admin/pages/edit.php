...
<main class="page">
    ...
    <form
        method="POST"
        action="<?= e(($basePath ?? '') . '/admin/pages/' . $page['id']) ?>"
        enctype="multipart/form-data">
        <?= \App\Core\Csrf::field() ?>

        <label for="template">Tipo de página</label>
        <select id="template" name="template">
            <option value="institucional" <?= $page['template'] === 'institucional' ? 'selected' : '' ?>>
                Institucional (empresa, serviços, sobre)
            </option>
            <option value="landing" <?= $page['template'] === 'landing' ? 'selected' : '' ?>>
                Página de vendas / captura de lead (simples)
            </option>
            <option value="profissional" <?= $page['template'] === 'profissional' ? 'selected' : '' ?>>
        ...
        <input type="text" id="subtitle" name="subtitle" value="<?= e($page['subtitle'] ?? '') ?>">

        <label for="hero_eyebrow">Rótulo pequeno acima do título (opcional)</label>
        <input type="text" id="hero_eyebrow" name="hero_eyebrow" value="<?= e($page['hero_eyebrow'] ?? '') ?>" placeholder="Ex: DEFESA EMPRESARIAL ESPECIALIZADA">
        ...
        <input type="text" id="title_highlight" name="title_highlight" value="<?= e($page['title_highlight'] ?? '') ?>">
        ...
        <select id="hero_layout" name="hero_layout">
            <option value="centered" <?= ($page['hero_layout'] ?? 'centered') === 'centered' ? 'selected' : '' ?>>Centralizado</option>
            <option value="card-right" <?= ($page['hero_layout'] ?? '') === 'card-right' ? 'selected' : '' ?>>Alinhado à esquerda, card à direita</option>
            <option value="card-left" <?= ($page['hero_layout'] ?? '') === 'card-left' ? 'selected' : '' ?>>Alinhado à direita, card à esquerda</option>
        </select>
        ...
        <textarea id="trust_badges" name="trust_badges" rows="3"><?= e($page['trust_badges'] ?? '') ?></textarea>
        ...
        <textarea id="content" name="content" rows="6"><?= e($page['content'] ?? '') ?></textarea>
        <?php if (!empty($errors['content'])): ?><p class="field-error flash"><?= e($errors['content']) ?></p><?php endif; ?>

        <label for="cta_text">
            Texto do botão de ação principal
            <small>(institucional: texto do botão. landing/profissional: título acima do formulário/WhatsApp)</small>
        </label>
        <input type="text" id="cta_text" name="cta_text" value="<?= e($page['cta_text'] ?? '') ?>">

        <label for="cta_link">
            Link do botão principal
            <small>(só usado no template institucional)</small>
        </label>
        <input type="text" id="cta_link" name="cta_link" value="<?= e($page['cta_link'] ?? '') ?>">

        <label for="cta_secondary_text">Botão secundário do hero — texto (opcional)</label>
        <input type="text" id="cta_secondary_text" name="cta_secondary_text" value="<?= e($page['cta_secondary_text'] ?? '') ?>" placeholder="Ex: Entender o procedimento">

        <label for="cta_secondary_link">Botão secundário do hero — link (opcional, padrão vai pro formulário de contato)</label>
        <input type="text" id="cta_secondary_link" name="cta_secondary_link" value="<?= e($page['cta_secondary_link'] ?? '') ?>" placeholder="#contato">
        ...
        <label for="image">Substituir imagem principal (opcional, JPG/PNG/WEBP/GIF, até 2MB)</label>
        ...
        <input type="text" id="image_caption" name="image_caption" value="<?= e($page['image_caption'] ?? '') ?>" placeholder="Ex: Nosso escritório em Cuiabá/MT">

        <label for="whatsapp">WhatsApp da página (formato: DDI+DDD+número, ex: 55659xxxxxxx)</label>
        <input type="text" id="whatsapp" name="whatsapp" value="<?= e($page['whatsapp'] ?? '') ?>" placeholder="556599999999">
        ...
        <input type="text" id="topbar_phone" name="topbar_phone" value="<?= e($page['topbar_phone'] ?? '') ?>" placeholder="(65) 99644-6210">
        ...
        <input type="text" id="topbar_note" name="topbar_note" value="<?= e($page['topbar_note'] ?? '') ?>">

        <?php if (!empty($page['logo'])): ?>
            <p>Logo atual:</p>
            <img
                src="<?= e(($basePath ?? '') . '/uploads/pages/' . $page['logo']) ?>"
                alt=""
                class="current-image"
                style="max-width:80px;">
        <?php endif; ?>

        <label for="logo">Substituir logo (opcional — se não enviar, usamos as iniciais do título automaticamente)</label>
        ...
        <input type="text" id="map_address" name="map_address" value="<?= e($page['map_address'] ?? '') ?>" placeholder="Rua Exemplo, 123 - Cidade/UF">
        ...
        <textarea id="privacy_notice" name="privacy_notice" rows="5"><?= e($page['privacy_notice'] ?? '') ?></textarea>
        ...
        <div id="sections-container">
            <?php foreach ($sections as $i => $s): ?>
                <div class="section-block">
                    ...
                    <input type="hidden" name="sections[<?= $i ?>][existing_image]" value="<?= e($s['image'] ?? '') ?>">

                    <label>Título da seção (opcional)</label>
                    <input type="text" name="sections[<?= $i ?>][title]" value="<?= e($s['title'] ?? '') ?>">

                    <label>Conteúdo</label>
                    <textarea name="sections[<?= $i ?>][content]" rows="4"><?= e($s['content']) ?></textarea>

                    <label>Cor de fundo (opcional)</label>
                    <input type="color" name="sections[<?= $i ?>][bg_color]" value="<?= e($s['bg_color'] ?? '#ffffff') ?>">

                    <label>Layout</label>
                    <select name="sections[<?= $i ?>][layout]">
                        <option value="text-only" <?= $s['layout'] === 'text-only' ? 'selected' : '' ?>>Somente texto</option>
                        <option value="checklist" <?= $s['layout'] === 'checklist' ? 'selected' : '' ?>>Checklist em grade (2 colunas)</option>
                        <option value="checklist-list" <?= $s['layout'] === 'checklist-list' ? 'selected' : '' ?>>Lista em coluna única (com divisórias)</option>
                        <option value="checklist-list-right" <?= $s['layout'] === 'checklist-list-right' ? 'selected' : '' ?>>Lista + card ao lado (card na direita)</option>
                        <option value="checklist-list-left" <?= $s['layout'] === 'checklist-list-left' ? 'selected' : '' ?>>Lista + card ao lado (card na esquerda)</option>
                        <option value="pain-card" <?= $s['layout'] === 'pain-card' ? 'selected' : '' ?>>Card escuro de dor/problema + CTA</option>
                        <option value="image-left" <?= $s['layout'] === 'image-left' ? 'selected' : '' ?>>Imagem à esquerda</option>
                        <option value="image-right" <?= $s['layout'] === 'image-right' ? 'selected' : '' ?>>Imagem à direita</option>
                    </select>
                    ...
                    <input type="text" name="sections[<?= $i ?>][aside_title]" value="<?= e($s['aside_title'] ?? '') ?>" placeholder="Ex: Essa situação está acontecendo com você?">
                    ...
                    <textarea name="sections[<?= $i ?>][aside_content]" rows="4"><?= e($s['aside_content'] ?? '') ?></textarea>

                    <?php if (!empty($s['image'])): ?>
                        <p>Imagem atual:</p>
                        <img src="<?= e(($basePath ?? '') . '/uploads/pages/' . $s['image']) ?>" class="current-image" alt="">
                        <label>
                            <input type="checkbox" name="sections[<?= $i ?>][remove_image]" value="1"> Remover esta imagem
                        </label>
                    <?php endif; ?>

                    <label>Substituir/adicionar imagem</label>
                    <input type="file" name="sections[<?= $i ?>][image]" accept="image/jpeg,image/png,image/webp,image/gif">
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div id="services-container">
            <?php foreach ($services as $i => $svc): ?>
                <div class="section-block">
                    ...
                    <input type="text" name="services[<?= $i ?>][icon]" value="<?= e($svc['icon'] ?? '') ?>" maxlength="4">
                    <label>Título</label>
                    <input type="text" name="services[<?= $i ?>][title]" value="<?= e($svc['title']) ?>">
                    <label>Descrição</label>
                    <textarea name="services[<?= $i ?>][description]" rows="3"><?= e($svc['description']) ?></textarea>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div id="team-container">
            <?php foreach ($team as $i => $m): ?>
                <div class="section-block">
                    ...
                    <input type="hidden" name="team[<?= $i ?>][existing_photo]" value="<?= e($m['photo'] ?? '') ?>">

                    <label>Nome</label>
                    <input type="text" name="team[<?= $i ?>][name]" value="<?= e($m['name']) ?>">

                    <label>Cargo/especialidade</label>
                    <input type="text" name="team[<?= $i ?>][role]" value="<?= e($m['role'] ?? '') ?>">

                    <label>WhatsApp direto (opcional)</label>
                    <input type="text" name="team[<?= $i ?>][whatsapp]" value="<?= e($m['whatsapp'] ?? '') ?>" placeholder="556599999999">

                    <?php if (!empty($m['photo'])): ?>
                        <p>Foto atual:</p>
                        <img src="<?= e(($basePath ?? '') . '/uploads/pages/' . $m['photo']) ?>" class="current-image" alt="">
                        <label>
                            <input type="checkbox" name="team[<?= $i ?>][remove_photo]" value="1"> Remover esta foto
                        </label>
                    <?php endif; ?>

                    <label>Substituir/adicionar foto</label>
                    <input type="file" name="team[<?= $i ?>][photo]" accept="image/jpeg,image/png,image/webp,image/gif">
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div id="faqs-container">
            <?php foreach ($faqs as $i => $faq): ?>
                <div class="section-block">
                    ...
                    <input type="text" name="faqs[<?= $i ?>][question]" value="<?= e($faq['question']) ?>">
                    <label>Resposta</label>
                    <textarea name="faqs[<?= $i ?>][answer]" rows="3"><?= e($faq['answer']) ?></textarea>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div id="steps-container">
            <?php foreach ($steps as $i => $step): ?>
                <div class="section-block">
                    ...
                    <input type="text" name="steps[<?= $i ?>][title]" value="<?= e($step['title']) ?>">
                    <label>Descrição</label>
                    <textarea name="steps[<?= $i ?>][description]" rows="3"><?= e($step['description']) ?></textarea>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <button type="submit" style="margin-top:2rem;">Salvar alterações</button>
    </form>
</main>

<script>
    (function() {
        function setupRepeater(containerId, templateId, addButtonId, removeClass, startIndex) {
        ...
        setupRepeater('sections-container', 'section-template', 'add-section', 'remove-section', <?= count($sections) ?>);
        setupRepeater('services-container', 'service-template', 'add-service', 'remove-service', <?= count($services) ?>);
        setupRepeater('team-container', 'team-template', 'add-team', 'remove-team', <?= count($team) ?>);
        setupRepeater('faqs-container', 'faq-template', 'add-faq', 'remove-faq', <?= count($faqs) ?>);
        setupRepeater('steps-container', 'step-template', 'add-step', 'remove-step', <?= count($steps) ?>);
    })();
</script>
