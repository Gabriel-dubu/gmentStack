...
<body class="pro-body">
        ...
        <div class="pro-nav-inner">
            <div class="pro-nav-identity">
                <?php if (!empty($page['logo'])): ?>
                    <img src="<?= e(($basePath ?? '') . '/uploads/pages/' . $page['logo']) ?>" alt=""
                        class="logo-badge logo-badge-image">
                <?php else: ?>
    ...
    <?php if ($hasImage || $hasContent): ?>
        <section class="pro-intro">
            <div class="pro-section-inner">
                <div
                    class="intro-block<?= !$hasContent ? ' intro-block-image-only' : '' ?><?= !$hasImage ? ' intro-block-text-only' : '' ?>">
                    <?php if ($hasImage): ?>
                        <figure class="intro-image-wrap">
                            <img src="<?= e(($basePath ?? '') . '/uploads/pages/' . $page['image']) ?>"
                                alt="<?= e($page['title']) ?>" class="intro-image">
        ...
        <section class="pro-section pro-section-alt">
            <div class="pro-section-inner">
                ...
                <?php foreach ($sections as $section): ?>
                    ...
                    <?php elseif ($layout === 'checklist-list-right' || $layout === 'checklist-list-left'): ?>
                        <section class="content-section">
                            <div
                                class="checklist-aside-wrap <?= $layout === 'checklist-list-left' ? 'checklist-aside-reverse' : '' ?>">
                                <div class="checklist-aside-list">
                                    <?php if (!empty($section['title'])): ?><h2><?= e($section['title']) ?></h2><?php endif; ?>
                                    <?= renderPlainList($section['content'], 'checklist-list-plain') ?>
                                </div>
                                <div class="checklist-aside-card">
                                    <?php if (!empty($section['aside_title'])): ?><h2><?= e($section['aside_title']) ?></h2>
                                    <?php endif; ?>
                                    <?php if (!empty($section['aside_content'])): ?>
                                        <?= renderPlainList($section['aside_content'], 'aside-bullets') ?>
                                    <?php endif; ?>
                    ...
                    <?php else: ?>
                        <section class="content-section content-section-<?= e($layout) ?>"
                            style="<?php if (!empty($section['bg_color'])): ?>background-color: <?= e($section['bg_color']) ?>;<?php endif; ?> color: <?= e($textColor) ?>;">
                            <div class="content-section-inner">
                                <?php if (!empty($section['image'])): ?>
                                    <img src="<?= e(($basePath ?? '') . '/uploads/pages/' . $section['image']) ?>"
                                        alt="<?= e($section['title'] ?? '') ?>" class="content-section-image">
                                <?php endif; ?>
                    ...
                    <?php foreach ($team as $member): ?>
                        ...
                        <div class="team-card">
                            <?php if (!empty($member['photo'])): ?>
                                <img src="<?= e(($basePath ?? '') . '/uploads/pages/' . $member['photo']) ?>"
                                    alt="<?= e($member['name']) ?>" class="team-photo">
                            <?php else: ?>
                                <div class="team-photo team-photo-placeholder">👤</div>
                            <?php endif; ?>
                            <h3><?= e($member['name']) ?></h3>
                            <?php if (!empty($member['role'])): ?><p class="team-role"><?= e($member['role']) ?></p>
                            <?php endif; ?>
                            <?php if ($memberLink): ?>
                                <a href="<?= e($memberLink) ?>" class="button button-whatsapp button-sm" target="_blank"
                                    rel="noopener">
        ...
        <div class="pro-section-inner pro-contact-inner">
            ...
            <?php else: ?>
                <form method="POST" action="<?= e(($basePath ?? '') . '/' . $page['slug'] . '/lead') ?>" class="lead-form"
                    ...
                    <?php if (!empty($leadErrors['name'])): ?><p class="field-error flash"><?= e($leadErrors['name']) ?></p>
                    <?php endif; ?>
                    ...
                    <input type="text" id="phone" name="phone" placeholder="(11) 91234-5678"
                        value="<?= e($leadOld['phone'] ?? '') ?>" required>
                    <?php if (!empty($leadErrors['phone'])): ?><p class="field-error flash"><?= e($leadErrors['phone']) ?>
                        </p><?php endif; ?>

                    <label for="email">E-mail (opcional)</label>
                    <input type="email" id="email" name="email" value="<?= e($leadOld['email'] ?? '') ?>">
                    <?php if (!empty($leadErrors['email'])): ?><p class="field-error flash"><?= e($leadErrors['email']) ?>
                        </p><?php endif; ?>
                    ...
                    <textarea id="message" name="message" rows="4"
                        placeholder="Não inclua senhas ou dados bancários."><?= e($leadOld['message'] ?? '') ?></textarea>

                    <button type="submit" class="button button-gold button-block">Quero ser atendido</button>

                    <div class="lead-form-alt-actions">
                        <button type="button" class="button-link" onclick="sendLeadViaWhatsapp()">Ou enviar por
                            WhatsApp</button>
    ...
    <?php if (!empty($page['map_address'])): ?>
        <section class="pro-map">
            <iframe src="https://www.google.com/maps?q=<?= rawurlencode($page['map_address']) ?>&output=embed" width="100%"
                height="360" style="border:0;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
        </section>
    <?php endif; ?>
    ...
    <?php if ($whatsappHero): ?>
        <a href="<?= e($whatsappHero) ?>" class="whatsapp-float" target="_blank" rel="noopener"
